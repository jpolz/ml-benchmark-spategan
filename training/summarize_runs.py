...
def extract_key_params(config):
    ...
    params['architecture'] = model.get('architecture', model.get('generator_architecture', 'N/A'))
...
def get_run_info(run_dir):
    ...
    params['has_final_model'] = (run_dir / 'final_models.pt').exists()
...
def detect_changes(runs):
    ...
    for param in first_run.keys():
        if param in ['timestamp', 'run_id', 'has_final_model', 'num_checkpoints', 'max_checkpoint_epoch']:
            continue
...
def generate_markdown_table(runs, show_all=False):
    ...
    training_params = ['batch_size', 'epochs', 'batches_per_epoch', 'early_stopping', 'patience']
    optimizer_params = ['gen_lr', 'gen_optimizer', 'gen_weight_decay', 
                       'disc_lr', 'disc_optimizer', 'disc_weight_decay']
    loss_params = ['loss_l1', 'loss_gan']
    data_params = ['domain', 'var_target', 'normalization']
    
    # Select parameters to show
    if show_all:
        params_to_show = (always_show + model_params + training_params + 
                         optimizer_params + loss_params + data_params)
    else:
...
def generate_summary_stats(runs):
    ...
    lines.append(f"- Runs with final models: {sum(1 for r in runs if r.get('has_final_model'))}")
    lines.append(f"- Runs without final models: {sum(1 for r in runs if not r.get('has_final_model'))}")
    ...
    # Architecture counts
    architectures = [r.get('architecture') for r in runs if r.get('architecture') != 'N/A']
    if architectures:
...
def main():
    ...
    parser.add_argument('--output', type=str, default='runs_summary.md',
                       help='Output markdown file (default: runs_summary.md)')
    parser.add_argument('--all', action='store_true',
                       help='Show all parameters, not just changed ones')
    parser.add_argument('--filter', type=str, default=None,
                       help='Filter runs by substring in run_id')
