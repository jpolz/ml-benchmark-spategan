...
def generate_ensemble_prediction(
    ...
    """
    x_tensor = x_tensor.to(device)
    doy_tensor = doy_tensor.to(device)
    ...
    with torch.no_grad():
        for _ in range(ensemble_size):
            ...
            output = model_wrapper.predict(x_tensor, doy_tensor)
...
def run_predictions_for_file(
    ...
    ds_test = xr.open_dataset(predictor_path).load()
    cf = config.load_config_from_yaml(model_wrapper.run_dir / "config.yaml")
    if domain == "SA":
    ...
    # order variables according to norm params to ensure correct alignment
    if 'min' in norm_params:
        keylist = list(norm_params['min'].keys())
    else:
        keylist = list(norm_params['std'].keys())
    if keylist != list(ds_test.data_vars):
        print("Reordering predictor variables to match normalization parameters")
        print("Order of predictor datavars before reordering", list(ds_test.data_vars))
        ds_test = ds_test[keylist]
        print("Order of predictor datavars after reordering", list(ds_test.data_vars))
    ...
    # TODO: switch to proper dataloader
    dataloader = build_dataloader_inference(cf, ds_normalized, batch_size)
    ...
    #     batch = x_tensor[i : i + batch_size]
        ...
        if ensemble_size is not None:
            preds = generate_ensemble_prediction(
                model_wrapper, batch, doy, device, ensemble_size
            )
        else:
...
def load_model_and_config(
    ...
    # Get normalization method and load parameters
    ...
    if normalization == "standardization":
        norm_params = {
            "mean": xr.open_dataset(run_dir / "x_mean.nc"),
            "std": xr.open_dataset(run_dir / "x_std.nc"),
        }
        ...
        norm_params = {
            "min": xr.open_dataset(run_dir / "x_min.nc"),
            "max": xr.open_dataset(run_dir / "x_max.nc"),
        }
    else:
        norm_params = {
            "mean": xr.open_dataset(run_dir / "x_mean.nc"),
...
def main():
    ...
    if args.config:
        ...
        print("No config file provided, please provide config")
...
def main_from_config(args):
    ...
    if isinstance(models_config, list):
        ...
        print("ERROR: Currently only list format for models is supported in config")
        return
